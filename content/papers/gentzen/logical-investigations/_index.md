---
title: "Untersuchungen über das logische Schließen. I"
weight: 2
bookCollapseSection: false
---

# Untersuchungen über das logische Schließen. I
**論理的推論についての研究 I**

**著者**: Gerhard Gentzen (Göttingen)  
**原題**: Untersuchungen über das logische Schließen. I  
**出版年**: 1935  
**掲載誌**: Mathematische Zeitschrift, Vol. 39

---

<details>
<summary>
<strong>概要<br>Overview</strong>
<span style="margin-left: 10px;">
<a href="slides/overview/" title="スライド" style="text-decoration: none; font-size: 1.2em;">📊</a>
<a href="https://youtu.be/dTHSC7XSSYc" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.2em; margin-left: 8px;">🎧</a>
</span>
</summary>

この論文は、現代の証明論において最も重要な論文の一つです。ゲンツェンは本論文で**自然演繹**（natürliches Schließen）と**シーケント計算**（LJ, LK）を導入し、さらに有名な**カット除去定理**（Hauptsatz）を証明しました。

</details>

---

## Inhaltsverzeichnis / 目次

<details>
<summary>
<strong>Dissertation I: Overview<br>博士論文 第I部 概観</strong>
<span style="margin-left: 10px;">
<a href="../doctoral-thesis/slides/part1-overview/" title="スライド" style="text-decoration: none; font-size: 1.2em;">📊</a>
<a href="https://youtu.be/jfIVIqyZrgg" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.2em; margin-left: 8px;">🎧</a>
</span>
</summary>

ゲンツェンの博士論文第I部の概観です。

</details>

<details>
<summary>
<strong>Dissertation I: Kapitel 1<br>博士論文 第I部 第1節</strong>
<span style="margin-left: 10px;">
<a href="../doctoral-thesis/slides/part1-chapter1/" title="スライド" style="text-decoration: none; font-size: 1.2em;">📊</a>
<a href="https://youtu.be/NsLxFy0-6bs" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.2em; margin-left: 8px;">🎧</a>
</span>
</summary>

ゲンツェンの博士論文第I部 第1節です。

</details>

<details>
<summary>
<strong>I. Abschnitt: Bezeichnungsfestsetzungen<br>第I節　記号の定め（記号規約）</strong>
<span style="margin-left: 10px;">
<a href="#" title="スライド" style="text-decoration: none; font-size: 1.2em;">📊</a>
<a href="#" title="音声解説" style="text-decoration: none; font-size: 1.2em; margin-left: 8px;">🎧</a>
</span>
</summary>

記号と表記法の規約

</details>

<details>
<summary>
<strong>II. Abschnitt: Der Kalkül des natürlichen Schließens<br>第II節　自然推論の計算体系</strong>
</summary>

<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin: 10px 0;">
  <div style="padding: 8px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§1</strong> Beispiele des natürlichen Schließens（自然推論の例）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter2-1/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/ePwjWDH2J6Q" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
  </div>
  <div style="padding: 8px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§2</strong> Aufstellung des Kalküls NJ（計算体系 NJ の構成）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter2-2/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/HDw0E7fa444" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
  </div>
  <div style="padding: 8px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§2 補足</strong> Aufstellung des Kalküls NJ（計算体系 NJ の構成）補足
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter2-2-supplement/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/wEUz3bV2Q5U" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
  </div>
  <div style="padding: 8px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§3</strong> Der inhaltliche Sinn der NJ-Schlußfiguren（NJ 結論図式の内容的意味）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter2-3/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/ksnX7BIwhcw" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
  </div>
  <div style="padding: 8px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§4</strong> Beschreibung der drei Beispiele aus §1 als NJ-Herleitungen（§1の3つの例をNJ導出として記述する）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter2-4/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/tmNyGiNEEBk" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
  </div>
  <div style="padding: 8px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§5</strong> Einige Bemerkungen zum Kalkül NJ. Der Kalkül NK.（計算体系NJに関する若干の注意 ― 計算体系NK）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter2-5/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/riC8fbj0u4A" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
  </div>
</div>

</details>

<details>
<summary>
<strong>III. Abschnitt: Die Schlußweisenkalküle LJ, LK und der Hauptsatz<br>第III節　推論計算体系 LJ・LK と主定理</strong>
</summary>

<div style="display: grid; grid-template-columns: 1fr; gap: 10px; margin: 10px 0;">
  <div style="padding: 10px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§1</strong> Die Kalküle LJ und LK（計算体系 LJ と LK）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter3-1/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/CwrRFrHLds0" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
    <div style="font-size: 0.9em; color: #444; margin-top: 8px; line-height: 1.5;">
      第2章で定義された自然演繹（NJ,NK）とは異なり、証明の途中で「仮定」を置いたり取り消したりする複雑さを避けた、新しい「ロジスティック」な計算体系である<strong>シークエント計算（LJ,LK）を導入する節です。前提部と結論部を「→」で結ぶシークエンス（Sequenz）</strong>という表現形式を定義し、論理記号を使わない「構造的推論図式（弱化、収縮、交換、カット）」と「論理記号推論図式」を整備しています。また、直観主義論理（LJ）と古典論理（LK）の違いを、特定の公理の有無ではなく、「シークエンスの右側（結論部）に置ける式の数」という構造的な制限の違いとして見事に定式化しています。
    </div>
  </div>

  <div style="padding: 10px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§2</strong> Einige Bemerkungen zu den Kalkülen LJ und LK. — Der Hauptsatz.（計算体系 LJ と LK に関するいくつかの注意。— 主定理）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter3-2/" title="スライド" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/jtUwj7Q5u0g" target="_blank" title="音声解説" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
    <div style="font-size: 0.9em; color: #444; margin-top: 8px; line-height: 1.5;">
      定義された LJ と LK の性質（例えば古典論理 LK が持つ鏡像的な対称性など）を考察した上で、論理学史において極めて重要な<strong>「主定理（Hauptsatz）」を宣言する節です。主定理とは、「すべての LJ- または LK-導出は、同じ終式を持ち、かつ『カット（Schnitt）』と呼ばれる推論図形が一切現れない導出に変換できる」という定理（カット除去定理）です。さらに、カットを含まない証明においては、証明過程に現れるすべての式が最終的な結論の部分式（Teilformel）</strong>にしかなり得ないという「部分式特性」が成り立つ意義について解説しています。
    </div>
  </div>

  <div style="padding: 10px; background-color: #f5f5f5; border-radius: 5px;">
    <strong>§3</strong> Beweis des Hauptsatzes（主定理の証明）
    <span style="margin-left: 10px;">
      <a href="../doctoral-thesis/slides/part1-chapter3-3-1/" title="スライド (1)" style="text-decoration: none; font-size: 1.1em;">📊</a>
      <a href="https://youtu.be/zxbhr0XQJSQ" target="_blank" title="音声解説 (1)" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
      <a href="../doctoral-thesis/slides/part1-chapter3-3-2/" title="スライド (2)" style="text-decoration: none; font-size: 1.1em; margin-left: 8px;">📊</a>
      <a href="https://youtu.be/BHwokhnIiCA" target="_blank" title="音声解説 (2)" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
      <a href="#" title="スライド (3)" style="text-decoration: none; font-size: 1.1em; margin-left: 8px;">📊</a>
      <a href="#" title="音声解説 (3)" style="text-decoration: none; font-size: 1.1em; margin-left: 5px;">🎧</a>
    </span>
    <div style="font-size: 0.9em; color: #444; margin-top: 8px; line-height: 1.5;">
      前の節で宣言された主定理を、実際に厳密な手続きによって証明する節です。証明の技術的な困難を避けるため、ゲンツェンはカットの変種である<strong>「混合（Mischung）」</strong>という新しい推論図式を導入します。そして、消去したい式の複雑さを表す「次数（Grad）」と、証明図の中でのその式の位置の深さを表す「階数（Rang）」という2つの指標を定義し、これらに対する二重の数学的帰納法を用いて、証明の木構造から混合（カット）を徐々に上へと押し上げ、最終的に完全に消去してしまう具体的なアルゴリズムを展開しています。
    </div>
  </div>
</div>

</details>

---

## 主定理 (Hauptsatz)

> **Hauptsatz:**  
> Jede LJ- bzw. LK-Herleitung läßt sich in eine LJ- bzw. LK-Herleitung mit der gleichen Endsequenz umwandeln, in welcher das Schnittverfahren nicht vorkommt.

**主定理：**  
任意の LJ または LK 導出は、同じ終結列を持ち、かつカット規則を含まない LJ または LK 導出へと変換できる。

---

この定理は「カット除去定理」として知られ、証明論における最も重要な結果の一つです。
